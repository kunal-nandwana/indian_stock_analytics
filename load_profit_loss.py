...
PROFIT_LOSS_DIR = "financial/2025-08-25/profit_loss"
...
PG_PASSWORD = "root"
PG_TABLE = "indian_stock_analytics.profit_loss"
...
if __name__ == "__main__":
    # Dynamically fetch all Parquet files in the profit_loss directory
    fs = gcsfs.GCSFileSystem()
    parquet_files = fs.glob(f"{BUCKET_NAME}/{PROFIT_LOSS_DIR}/*/*.parquet")
    ...
    for file_path in parquet_files:
        ...
        try:
            ...
            # Debugging steps for 'year' column
            print("Columns in the DataFrame:", df.columns)
            print("Data types:", df.dtypes)

            print("Year column values:")    
            print(df['year'])
            print("Datatype of 'year' column:", df['year'].dtype)

        except Exception as e:
            print(f"  ✗ Could not load data from {gcs_path} ({e}), skipping.")
            continue

        # Debugging: Inspect 'year' column after transformation
        logging.info(f"Transformed 'year' column: {df['year'].head()}")
